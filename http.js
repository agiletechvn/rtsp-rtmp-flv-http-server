...
(function () {
  ...
  HTTPHandler = class HTTPHandler {
    ...
    handlePath(req, callback) {
      const pathname = req.uri.pathname;
      const query = querystring.parse(req.uri.query);
          ...
          (err, template) => {
            ...
            if (err) {
              ...
              if (query.output === 'json') {
                return this.respondText(JSON.stringify(opts), req, callback);
              } else {
...
`;
    ...
    respondText(str, req, callback, headers = null) {
      return this.respondTextWithHeader(str, req, headers, callback);
    }
                  ...
                  if (req.headers.range != null) {
                    if (
                      ...
                      if (from == null && to != null) {
                        // last n bytes
                        contentRangeHeader = `bytes ${contentBuf.length - to}-${
                          contentBuf.length - 1
                        }/${contentBuf.length}`;
