...
(function() {
  ...
  var DHKeyGenerate,
    ...
    MESSAGE_FORMAT_1,
    MESSAGE_FORMAT_2,
    MESSAGE_FORMAT_UNKNOWN,
    RTMP_SIG_SIZE,
    RandomCrud,
    SHA256DL,
    codec_utils,
    crypto,
    detectClientMessageFormat,
    generateS0S1S2,
    generateS1,
    generateS2,
    hasSameBytes,
  ...
  GenuineFPConstCrud = Buffer.concat([
  ...
  generateS1 = function(messageFormat, dh, callback) {
    return crypto.pseudoRandomBytes(RTMP_SIG_SIZE - 8, function(
      err,
      randomBytes
    ) {
      var handshakeBytes,
  ...
  generateS2 = function(messageFormat, clientsig, callback) {
    ...
    challengeKey = clientsig.slice(
      challengeKeyOffset,
      +(challengeKeyOffset + 31) + 1 || 9e9
    );
    if (messageFormat === 1) {
    ...
    return crypto.pseudoRandomBytes(RTMP_SIG_SIZE - 32, function(
      err,
      randomBytes
    ) {
    ...
    return generateS1(messageFormat, dh, function(err, s1Bytes) {
      return generateS2(messageFormat, clientsig, function(err, s2Bytes, keys) {
        ...
        allBytes = Buffer.concat(
          [
            Buffer.from([clientType]), // version (S0)
            s1Bytes, // S1
            s2Bytes // S2
          ],
          3073
