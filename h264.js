...
(function() {
  ...
  var ASPECT_RATIO_IDC_EXTENDED_SAR,
    Bits,
    RANGE_TAB_LPS,
    SLICE_TYPES,
    SUB_WIDTHS,
  ...
  api = {
    ...
    isPictureNALUnitType: function(nalUnitType) {
      return (
        nalUnitType === api.NAL_UNIT_TYPE_NON_IDR_PICTURE ||
        nalUnitType === api.NAL_UNIT_TYPE_IDR_PICTURE
      );
    },
    getSPS: function() {
      return sps;
    ...
    readPPS: function(nalUnit) {
      ...
      if (pps.num_slice_groups_minus1 > 0) {
        ...
        if (slice_group_map_type === 0) {
          for (
            iGroup = k = 0, ref = pps.num_slice_groups_minus1;
            0 <= ref ? k <= ref : k >= ref;
            iGroup = 0 <= ref ? ++k : --k
          ) {
          ...
          for (
            iGroup = l = 0, ref1 = pps.num_slice_groups_minus1;
          ...
          for (
            i = m = 0, ref2 = pic_size_in_map_units_minus1;
            0 <= ref2 ? m <= ref2 : m >= ref2;
            i = 0 <= ref2 ? ++m : --m
          ) {
            // Ceil( Log2( num_slice_groups_minus1 + 1 ) )
            numBits = Math.ceil(
              Math.log(pps.num_slice_groups_minus1 + 1) / Math.LN2
            );
        ...
        if (pic_scaling_matrix_present_flag === 1) {
          for (
            i = n = 0,
              ref3 =
                6 +
                (sps.chroma_format_idc !== 3 ? 2 : 6) * transform_8x8_mode_flag;
    ...
    getFrameSize: function(sps) {
      ...
      width =
        ...
        cropUnitX * sps.frame_crop_left_offset +
        1;
      height =
        16 * sps.frameHeightInMbs -
        (cropUnitY * sps.frame_crop_bottom_offset + 1) -
        cropUnitY * sps.frame_crop_top_offset +
        1;
      return {
        width: width,
        height: height
    ...
    readSPS: function(nalUnit) {
      ...
      if (reserved_zero_2bits !== 0) {
        throw new Error(
          `video error: reserved_zero_2bits must be 00: ${reserved_zero_2bits}`
        );
      }
      sps.level_idc = bits.read_byte();
      seq_parameter_set_id = bits.read_ue();
      if (
        (ref = sps.profile_idc) === 100 ||
        ref === 110 ||
        ref === 122 ||
        ref === 244 ||
        ref === 44 ||
        ...
        if (seq_scaling_matrix_present_flag) {
          for (
            i = k = 0, ref1 = sps.chroma_format_idc !== 3 ? 8 : 12;
      ...
      if (sps.pic_order_cnt_type === 0) {
        ...
        for (
          i = l = 0, ref2 = num_ref_frames_in_pic_order_cnt_cycle;
      ...
      if (zero_bits_sum !== 0) {
        logger.warn(
          "warn: malformed SPS data: rbsp_alignment_zero_bit must be all zeroes"
        );
      }
    ...
    read_slice_data: function(bits, opts) {
      ...
      currMbAddr =
        opts.sliceHeader.first_mb_in_slice *
        (1 + opts.sliceHeader.mbaffFrameFlag);
      moreDataFlag = 1;
      prevMbSkipped = 0;
      return (sliceTypeString = api.getSliceTypeString(
        opts.sliceHeader.slice_type
      ));
    },
    //    loop
    ...
    read_pred_weight_table: function(bits, opts) {
      ...
      chroma_offset_10 = [];
      for (
        i = k = 0, ref = sliceHeader.num_ref_idx_l0_active_minus1;
        0 <= ref ? k <= ref : k >= ref;
        i = 0 <= ref ? ++k : --k
      ) {
      ...
      if (sliceHeader.slice_type % 5 === 1) {
        ...
        results = [];
        for (
          i = m = 0, ref1 = sliceHeader.num_ref_idx_l1_active_minus1;
    ...
    read_dec_ref_pic_marking: function(bits, opts) {
      var adaptive_ref_pic_marking_mode_flag,
    ...
    read_slice_header: function(bits, opts) {
      ...
      if (sps.pic_order_cnt_type === 0) {
        sliceHeader.pic_order_cnt_lsb = bits.read_bits(
      ...
      if (
        (pps.weighted_pred_flag &&
          (sliceTypeString === "P" || sliceTypeString === "SP")) ||
      ...
      if (
        ...
        3 <= slice_group_map_type &&
        slice_group_map_type <= 5
      ) {
        // Ceil( Log2( PicSizeInMapUnits + SliceGroupChangeRate + 1 ) )
        numBits = Math.ceil(
          Math.log(
            sps.picSizeInMapUnits + pps.slice_group_change_rate_minus1 + 1
          ) / Math.LN2
        );
    ...
    parseNALUnit: function(nalUnit) {
      ...
      if (
        (ref = data.nalHeader.nal_unit_type) ===
          api.NAL_UNIT_TYPE_NON_IDR_PICTURE ||
        ref === api.NAL_UNIT_TYPE_IDR_PICTURE
      ) {
    ...
    removeEmulationPreventionByte: function(nalUnit) {
      ...
      removeBytePositions = [];
      while (true) {
        emulPos = Bits.searchBytesInArray(
          nalUnit,
          [0x00, 0x00, 0x03],
          searchPos
        );
      ...
      if (removeBytePositions.length > 0) {
        newBuf = Buffer.alloc(nalUnit.length - removeBytePositions.length);
        currentSrcPos = 0;
        ...
        if (currentSrcPos < nalUnit.length) {
          nalUnit.copy(
            newBuf,
            currentSrcPos - removeBytePositions.length,
            currentSrcPos,
            nalUnit.length
          );
        }
    ...
    read_hrd_parameters: function(bits) {
      ...
      cbr_flag = [];
      for (
        schedSelIdx = k = 0, ref = cpb_cnt_minus1;
        0 <= ref ? k <= ref : k >= ref;
        schedSelIdx = 0 <= ref ? ++k : --k
      ) {
    ...
    concatWithStartCodePrefix: function(bufs) {
      ...
      startCodePrefix = Buffer.from([0x00, 0x00, 0x00, 0x01]);
      totalLen = 0;
    ...
    readAVCDecoderConfigurationRecord: function(bits) {
      ...
      if (info.configurationVersion !== 1) {
        throw new Error(
          `configurationVersion is not 1: ${info.configurationVersion}`
        );
      }
      // SPS[1..3]
    ...
    parseSpropParameterSets: function(str) {
      ...
      for (k = 0, len = ref.length; k < len; k++) {
        base64String = ref[k];
        nalUnits.push(Buffer.from(base64String, "base64"));
      }
      return nalUnits;
    }
  };
